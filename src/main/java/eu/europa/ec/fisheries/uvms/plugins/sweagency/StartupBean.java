...
import eu.europa.ec.fisheries.uvms.plugins.sweagency.mapper.ServiceMapper;
import eu.europa.ec.fisheries.uvms.plugins.sweagency.producer.PluginMessageProducer;
import eu.europa.ec.fisheries.uvms.plugins.sweagency.service.FileHandlerBean;
...
public class StartupBean extends PluginDataHolder {
    ...
    public void startup() {
        ...
        serviceType = ServiceMapper.getServiceType(
                ...
                getApplicationName(),
                "Plugin for sending reports to HaV endpoints",
                PluginType.OTHER,
                getPluginResponseSubscriptionName());

        register();

        LOG.debug("Settings updated in sweagency-plugin {}", REGISTER_CLASS_NAME);
